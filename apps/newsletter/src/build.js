...
<head>
  ...
  <style>
    ...
    a { color: inherit; }
    .shell { max-width: 680px; margin: 0 auto; padding: 32px 20px 48px; }
    header { display: flex; flex-direction: column; gap: 12px; border-bottom: 1px solid #ddd; padding-bottom: 20px; }
...
async function generateHomepage(posts) {
  ...
  const fallbackSection = { title: "Latest", posts: posts.slice(0, PAGE_SIZE) };

  const blocks = (sections.length ? sections : [fallbackSection]).map((section) => {
    const items = section.posts?.length ? section.posts : posts.slice(0, PAGE_SIZE);
    const entries = items
      .map((post) => {
        const dateValue = post.published_at || post.publish_at || post.created_at;
        return `<article class="newsletter-item">
          <h2><a href="/${post.slug}/">${escapeHtml(post.title || post.slug)}</a></h2>
          <div class="muted">${escapeHtml(formatDateLabel(dateValue))}</div>
          <p>${escapeHtml(post.excerpt || "")}</p>
        </article>`;
      })
      .join("");
    return `<section>
      <h2>${escapeHtml(section.title || "Updates")}</h2>
      ${entries || "<p>게시물이 없습니다.</p>"}
    </section>`;
  });
...
async function generateFeed(posts) {
  const items = posts.slice(0, 20).map((post) => {
    const link = buildUrl(siteBaseUrl, `/${post.slug}/`);
    const pubDate = formatRssDate(post.published_at || post.publish_at || post.created_at);
    return `<item>
  <title>${escapeHtml(post.title || post.slug)}</title>
...
<channel>
  ...
  <link>${escapeHtml(siteBaseUrl)}</link>
  <description>${escapeHtml(siteConfig.footer_text || "Newsletter feed")}</description>
  ${items.join("\n")}
</channel>
</rss>
`;
  await writeHtml(path.join(DIST_DIR, "feed.xml"), xml);
...
async function build() {
  ...
  siteTitle = process.env.SITE_TITLE || meta?.tenantSlug || "Newsletter";
  ...
  await generateFeed(postEntries);
