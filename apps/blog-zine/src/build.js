...
const FALLBACK_IMAGE = `data:image/svg+xml;utf8,${encodeURIComponent(
  `<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="800">
    <rect width="100%" height="100%" fill="#f4f0ff"/>
    <circle cx="200" cy="200" r="120" fill="#ffeb3b" opacity="0.6"/>
    <circle cx="1000" cy="600" r="160" fill="#00e5ff" opacity="0.5"/>
    <rect x="80" y="80" width="1040" height="640" fill="none" stroke="#111" stroke-width="6" opacity="0.3"/>
  </svg>`
)}`;
...
function resolvePostImage(post) {
  const candidate =
    ...
    post.thumbnail_url ||
    "";
  return sanitizeUrl(candidate, { allowDataImage: true }) || FALLBACK_IMAGE;
}
...
async function generatePostPages(posts) {
  ...
  <h2 class="zine-title"><span>${escapeHtml(post.title || post.slug)}</span></h2>
...
async function generateHomepage(posts) {
  ...
  const [featured, ...restPosts] = latestPosts;
  const hero = featured
    ? `<section class="hero-feature">
  <img src="${escapeHtml(resolvePostImage(featured))}" alt="${escapeHtml(
        featured.title || featured.slug
      )}" />
  <div class="overlay">
    <span class="badge">Cover Story</span>
    <h2 class="zine-title"><span>${escapeHtml(
      featured.title || featured.slug
    )}</span></h2>
    <p>${escapeHtml(featured.excerpt || "")}</p>
    <a href="/${featured.slug}/">Read full story →</a>
  </div>
</section>`
    : "";
  ...
  ${
    hasPosts
      ? `<h2 class="zine-title"><span>Latest Issue</span></h2>
  <ul class="post-list">
${restPosts
  .map(
    (post) =>
...
async function generateCategoryPages(posts, categories) {
  ...
  <p>${escapeHtml(post.excerpt || "")}</p>
      ...
      const html = layoutHtml({
        ...
        content: `<h2 class="zine-title"><span>${escapeHtml(
